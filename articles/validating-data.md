...
application. As a result, understanding schemas is central to using RV
effectively. Most of this vignette focuses on constructing schemas, with
the final section demonstrating how to use them for data validation.
...
vignette](https://lj-jenkins.github.io/RV/articles/validation-rules.md)
and the adding rules vignette.
...
Data validation in RV is undertaken with the `Validator` class, which
ingests a `Schema` and applies the rules within to the input data.
...
rules that use `.data` to access other data nodes will be accessing the
data state at that point of the schema walk, rather than the original
state of the data. See the following example:
...
SPSS, SAV, Excel, etc. -\> data.frame -\> RV.
...
RV can validate any list, data.frame, or atomic vector.
